...
def prepare_latent(latents, model, height, width, generator, batch_size):
    if latents is None:
        latents = torch.randn(
...
def run_sample(
    ...
    latents = prepare_latent(latents, model, height, width, generator, batch_size)
...
class DiffusersSDModelBase(DiffusersModelBase):
    ...
    #     timesteps, _ = retrieve_timesteps(self.pipe.scheduler, kwargs.get('num_inference_steps', self.num_inference_steps), device=self.pipe.device, timesteps=kwargs.get('timesteps', None))
    #     timesteps = timesteps[int(round(start_after_relative * len(timesteps))):int(round(cutoff_after_relative * len(timesteps)))]
    #     return run_sample(model=self.pipe, controller=controller, **(self._get_pipe_kwargs(embs, embs_neg, start_sample=start_sample, **kwargs) | { 'timesteps': timesteps, 'num_inference_steps': len(timesteps) }))

    # @torch.no_grad
    # def sample_delayed(self, controller, embs: List[PromptEmbedding], embs_unmodified: List[PromptEmbedding], embs_neg: Optional[List[PromptEmbedding]], embs_neg_unmodified: Optional[List[PromptEmbedding]] = None, start_sample: Optional[Float[torch.Tensor, 'n c h w']] = None, delay_relative: float = .2, **kwargs) -> Union[List[Image.Image], Any]:
    #     if delay_relative == 0:
    #         return self.sample(controller=controller, embs=embs, embs_neg=embs_neg, start_sample=start_sample, **kwargs)
    #     intermediate = self.sample(controller=controller, embs=embs_unmodified, embs_neg=(embs_neg_unmodified if not embs_neg_unmodified is None else embs_neg), start_sample=start_sample, **kwargs, output_type='latent', cutoff_after_relative=delay_relative)
