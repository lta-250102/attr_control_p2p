...
def main(cfg: DictConfig):
    ...
    if mode == 'gen':
        ...
        img.save(f'{cfg.out_dir}{mode}/{cfg.get('img_name', str(datetime.now))}.jpg')
    elif mode == 'edit':
        ...
        os.makedirs(f'{cfg.out_dir}{mode}/{cfg.get('img_name', str(datetime.now))}/', exist_ok=True)
        for i in range(len(imgs)):
            attr_name = list(alphas.keys())[i]
            imgs[i].save(f'{cfg.out_dir}{mode}/{cfg.get('img_name', str(datetime.now))}/{i+1}_{attr_name}_{alphas[attr_name]}.jpg')
    elif mode == 'both':
        alphas: dict = dict(zip(cfg.deltas, cfg.alphas))
        ...
        for i in range(len(imgs)):
            ...
            alphas['ori'] = 0
            imgs[i].save(f'{cfg.out_dir}{mode}/{cfg.get('img_name', str(datetime.now))}/{i}_{attr_name}_{alphas[attr_name]}.jpg')
    else: 
        print("Invalid mode!!")
